<script>
    ...
    const draw = (JXG, id) => {
		...
		for (i=0; i<number.Value();i++) { pointlist.push(brd.create('point', [200*Math.random() , 30*Math.random()+20 ], { style: 4, color: 'black', name:" "})); }
		...
		let t = brd.create('text',[20,-40, function(){ return "r(x) = " + regression.getTerm();} ],{strokeColor:'black',fontSize:'14px'});
		...
		function numberPoints() {
			...
			if (!(number.Value() == 2 && degree.Value() >= 1)) {
				for (i=0; i<pointlist.length; i++){ brd.removeObject(pointlist[i]) }
				pointlist = [];
				for (i=0; i<number.Value();i++) { pointlist.push(brd.create('point', [200*Math.random() , 30*Math.random()+20 ], { style:4, color:"black", name:" "})); }
				regression = JXG.Math.Numerics.regressionPolynomial(degree, pointlist);
				brd.removeObject(reg)
				reg = bobrdard.create('functiongraph',[regression], {strokeColor:'red'});
			}
