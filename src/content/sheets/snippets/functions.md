...
console.log(
	'exp(x) = 7.38905609893065 <=> x =',
	dichotomy((x) => Math.exp(x), 7.38905609893065, 0, 10, 0.000000001)
);
```
...
fixpoint = (fn, start, delta) => {
	...
	iter = (fx, value, delta, c = 0) => {
		if (c > MAX_ITER)
			throw `Method doesn't converges with the start at ${start} in the allowed duration (${MAX_ITER} iters)`;
